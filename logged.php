<?php 

    require_once 'dbconfig.php';
    require_once 'auth.php';
    if (!$email = checkAuth()) {
        header("Location: login.php");
        exit;
    }
    else{

        $conn = mysqli_connect($dbconfig['host'], $dbconfig['user'], $dbconfig['password'], $dbconfig['name']);
        $email = mysqli_real_escape_string($conn, $_SESSION['email']);
        $query = mysqli_query($conn,  "SELECT * FROM users WHERE email = '".$email."'");
       
        while($r=mysqli_fetch_assoc($query)){
            $nome= $r["nome"];
            $cognome=$r["cognome"];
            $data=$r["data_nascita"];
            $email=$r["email"];
            $provincia = $r["provincia"];
            $nazione = $r["nazione"];
            $citta = $r["citta"];
            $n_famiglia = $r["n_famiglia"];
            $indirizzo =$r["indirizzo"];
            $lavoro = $r["lavoro"];
            $genere = $r["genere"];
            $istruzione =$r["istruzione"];
            $cap=$r["cap"];
        }
    }


    if(isset($_POST["nome"]) && isset($_POST["cognome"]) && isset($_POST["email"])){

            $nuovo_nome=mysqli_real_escape_string($conn, $_POST["nome"]);
            $nuovo_cognome=mysqli_real_escape_string($conn, $_POST["cognome"]);
            $nuova_mail=mysqli_real_escape_string($conn, $_POST["email"]);
            $nuovo_cap=mysqli_real_escape_string($conn, $_POST["cap"]);
            $nuova_provincia=mysqli_real_escape_string($conn, $_POST["provincia"]);
            $nuova_nazione=mysqli_real_escape_string($conn, $_POST["nazione"]);
            $nuova_citta=mysqli_real_escape_string($conn, $_POST["citta"]);
            $nuova_famiglia=mysqli_real_escape_string($conn, $_POST["n_famiglia"]);
            $nuovo_indirizzo=mysqli_real_escape_string($conn, $_POST["indirizzo"]);
            $nuovo_lavoro=mysqli_real_escape_string($conn, $_POST["lavoro"]);
            $nuovo_genere=mysqli_real_escape_string($conn, $_POST["genere"]);
            $nuova_istruzione=mysqli_real_escape_string($conn, $_POST["istruzione"]);
            
            $query_modifica = "UPDATE users SET nome ='".$nuovo_nome."',  cognome = '".$nuovo_cognome."',
            provincia='".$nuova_provincia."',  nazione='".$nuova_nazione."', citta='".$nuova_citta."', n_famiglia=".$nuova_famiglia.", indirizzo='".$nuovo_indirizzo."',
            lavoro='".$nuovo_lavoro."', genere='".$nuovo_genere."', istruzione='".$nuova_istruzione."', cap ='".$nuovo_cap."'
            WHERE email='".$_SESSION["email"]."' ";
            
            $res=mysqli_query($conn,  $query_modifica);
               
            $nome=$nuovo_nome;
            $cognome=$nuovo_cognome;
            $email=$nuova_mail;
            $provincia = $nuova_provincia;
            $nazione = $nuova_nazione;
            $citta = $nuova_citta;
            $n_famiglia = $nuova_famiglia;
            $indirizzo = $nuovo_indirizzo;
            $lavoro = $nuovo_lavoro;
            $genere = $nuovo_genere;
            $istruzione = $nuova_istruzione;
            $cap = $nuovo_cap;

    }
    
    
?>

<!DOCTYPE>
<html>

    <head>
        <title>Pavesini</title>
        <link rel="stylesheet" href="logged.css">
        <script src="logged.js" defer></script>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@24,400,0,0" />
        <meta name="viewport" content="width=device-width, initial-scale=1">
    </head>

    <div ><img id="immagine_omino" src='icons8-accessibilità-2-50.png'></div>

    <body>
        
        <header>
            <nav>
            <div id="omino-login">
                <a href="login.php"></a>
            </div>
            
            <div>
                <img id="logo" src="https://www.pavesini.it/wp-content/themes/pavesini-new/images/logo_desktop.png">
            </div>
            <div id="barra">
                <a href="index.php">home</a>
                <a  href="shop.php">pavesi</a>
                <a id="prodotto" href="https://www.pavesini.it/prodotti/">prodotti</a>
                <div class="hidden" >
                     <div id="prodotti"> 
                        <a href="https://www.pavesini.it/pavesini/snack/">SNACK </br></a>
                        <a href="https://www.pavesini.it/pavesini/gelati/">GELATI</a>
                    </div>
                </div>
                <a  id="concorso">concorsi </a>
                <div id="vinciconc" class="hidden" > 
                    <a href="https://www.pavesini.it/concorsi/vinci-e-parti-2024/">VINCI</br> E </br>PARTI</br> </a>
                </div>
                <a href="https://www.pavesini.it/ricette/">ricette</a>
            </div>

            <div id="icon">

                <div class="border_icone">
                    <a href="https://www.facebook.com/pavesiniofficial">
                        <img src="data:image/png;base64,">
                    </a>
                </div>

                <div class="border_icone">
                    <a href="https://www.instagram.com/pavesini_official/">
                        <img src="https://img.icons8.com/material-outlined/100/0051a3/instagram-new--v1.png" alt="instagram-new--v1"/>
                    </a>
                </div>

                <div class="border_icone">
                    <a href="https://consent.youtube.com/m?continue=https%3A%2F%2Fwww.youtube.com%2Fchannel%2FUCoTAAq6525AWdY3qHbILoyw%3Fcbrd%3D1&gl=IT&m=0&pc=yt&cm=2&hl=it&src=1">
                        <img src="data:image/png;base64,">
                    </a>
                </div>

                <div class="border_icone">
                    <a href="https://open.spotify.com/user/314dxbsruiaun4dtinu2tjb7msya">
                        <img src="data:image/png;base64,">
                    </a>
                </div>  

                <div class="border_icone">
                    <a href="login.php">
                        <img src="omino.png">
                    </a>
                </div>  

                <div class="border_icone">
                    <a href="favorite.php">
                        <img  class="blue" src="cuore.png">
                    </a>
                </div>  

            </div>

                <div id="menu">
                    <div></div>
                    <div></div>
                    <div></div>
                </div>
            </nav>

            <div id="showmenu" class="hidden">
                <a href="">home</a>
                <div id="linea_menu">
                    <div></div>
                    <a>prodotti</a>
                    <a href="https://www.pavesini.it/pavesini/snack/">snack</a>
                    <a href="https://www.pavesini.it/pavesini/gelati/">gelati</a>
                </div>
                <div id="linea_menu2">
                    <div></div>
                    <a href="">concorsi</a>
                    <a href="https://www.pavesini.it/concorsi/vinci-e-parti-2024/">vinci e parti</a>
                </div>
                <a href="https://www.pavesini.it/ricette/">ricette</a>
            </div>
    

        </header>
        
        <section id="centrale">
        <section id=main-dati>
            <div id="home-profilo">
                <a href="index.php">Home  </a>
                <a> >> Profilo</a>
            </div>
            <div class="ciao-user">
                <h1> Ciao <?php echo $nome ?></h1>
                <div>
                    <strong>
                    <a href="">Gestione privacy e newsletter ></a>
                    <a href="">concorsi ></a>
                    <a href="logout.php">logout ></a>
                    </strong>
                </div>
            </div>
                <h1 class="centra">I miei dati</h1>
                <form method="post">
                    <div class="riga">   
                     
                        
                        <div class="sinistra">

                            <p>Nome</p>
                            <input type="text" name="nome" value= <?php echo $nome ?> >


                            <p>Email</p>
                            <input type= "text" name="email" value= <?php echo $email ?>>

                            <p>Nazione</p>
                            <input type= "text" name="nazione" value= <?php echo $nazione ?>>

                            <p>Città</p>
                            <input type="text" name="citta" value= <?php echo $citta ?>>
        
                            <p>Indirizzo</p>
                            <input type="text" name="indirizzo" value= <?php echo $indirizzo ?>>

                            <p>Genere</p>
                            <input type="text" name="genere" value= <?php echo $genere ?>>
                            
                        </div>
                            
                        
                        <div class="destra">

                            <p>Cognome</p>
                            <input type="text" name="cognome" value= <?php echo $cognome ?>>

                            <p>Provincia</p>
                            <input type="text" name="provincia" value= <?php echo $provincia ?>>
                        
                            <p>Cap</p>
                            <input type="text" name="cap" value= <?php echo $cap ?>>

                        
                            <p>Da quante persone è composto il tuo nucleo familiare</p>
                            <input type="text" name="n_famiglia" value= <?php echo $n_famiglia ?>>


                            <p>Istruzione</p>
                            <input type="text" name="istruzione" value= <?php echo $istruzione ?>>
                    
                            <p>In quale ambito lavori o eserciti le tue passioni</p>
                            <input type="text" name="lavoro" value= <?php echo $lavoro ?>>
                        </div>
                        
                    </div>
                    <div class="centra">
                          <input class="salva" type="submit" value="Salva">
                    </div>
                  
                </form>
        </section>
        </section>
        
            <!--FOOTER-->
            <footer>
                <img src="	https://www.pavesini.it/wp-content/themes/pavesini-new/images/logo-pavesini-footer.png" >

                <div>
                    <a href="https://www.pavesini.it/dati-societari/">Dati Societari
                    <a href="https://www.pavesini.it/privacy/">Privacy policy</a>
                    <a href="https://www.pavesini.it/cookies/">Cookie policy</a>
                    <a href=""> Gestisci</a>
                    <a href="https://www.pavesini.it/contatti/">Contatti</a>
                    <a href="https://www.barillagroup.com/it/di-piu-su-di-noi/barilla-gdpr-policy/">Barilla - GDPR </a>
                    <a href="https://www.pavesini.it/wp-content/uploads/2023/05/DDA-PAVESINI.pdf"> Dichiarazione di accessibilità</a>
                </div> 

                <div id="allinea">
                    <p>
                        <div class="scopri-ricette-font"><strong>Scopri le nostre ricette:</strong> </div> 
                        <div>
                            <a class="sottolinea" href="https://www.pavesini.it/ricette/tiramisu-al-limone/">Tiramisù al limone</a>
                            <a class="sottolinea" href="https://www.pavesini.it/ricette/mattonella-dolce/"> Mattonella dolce</a>
                            <a class="sottolinea" href="https://www.pavesini.it/ricette/zuccotto-semifreddo/">Zuccotto semifreddo</a><br>
                            <a class="sottolinea" href="https://www.pavesini.it/ricette/tiramisu-alle-fragole-con-pavesini/">Tiramisù alle fragole con Pavesini</a>
                        </div>
                    </p>
                </div>
                
                <div id="barilla">
                    <strong> © 2024 Barilla - P. IVA 01654010345</strong>
                </div>
            </footer>

        </section>
            <div id="icone">
            
                    <div class="border_icon">
                        <a href="https://www.facebook.com/pavesiniofficial">
                            <img src="data:image/png;base64,">
                        </a>
                    </div>

                    <div class="border_icon">
                        <a href="https://www.instagram.com/pavesini_official/">
                            <img src="https://img.icons8.com/material-outlined/100/0051a3/instagram-new--v1.png" alt="instagram-new--v1"/>
                        </a>
                    </div>

                    <div class="border_icon">
                        <a href="https://consent.youtube.com/m?continue=https%3A%2F%2Fwww.youtube.com%2Fchannel%2FUCoTAAq6525AWdY3qHbILoyw%3Fcbrd%3D1&gl=IT&m=0&pc=yt&cm=2&hl=it&src=1">
                            <img src="data:image/png;base64,"> 
                    </div>

                    <div class="border_icon">
                        <a href="https://open.spotify.com/user/314dxbsruiaun4dtinu2tjb7msya">
                            <img src="data:image/png;base64,">
                        </a>
                    </div>

            </div>

    </section>
      </body>
</html>